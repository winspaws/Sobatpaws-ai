"""Registry identitas entitas Ekosistem Satwa — penghubung ID app utama ↔ sesi ML/AI.
...
class EkosistemSatwaEntityIds(BaseModel):
    ...
    external_consultation_id: str | None = Field(
        None, description="ID konsultasi/kasus dari app Ekosistem Satwa utama",
    )
    ...
    def _sync_aliases(self) -> EkosistemSatwaEntityIds:
    ...
    *,
    ...
    if not ext and ctx.external_refs:
        ext = ctx.external_refs.get("consultation_id") or ctx.external_refs.get(
            "ekosistem_satwa_consultation_id"
        )
    return EkosistemSatwaEntityIds(
...
class IdentityRegistry:
    """Persist mapping consultation_id ↔ entitas Ekosistem Satwa (JSONL)."""
    ...
    def register(
        self,
        consultation_id: str,
        entities: EkosistemSatwaEntityIds | dict,
        *,
        context_snapshot: dict | None = None,
    ) -> dict[str, Any]:
        if isinstance(entities, EkosistemSatwaEntityIds):
    ...
    def get_entities(self, consultation_id: str) -> EkosistemSatwaEntityIds | None:
