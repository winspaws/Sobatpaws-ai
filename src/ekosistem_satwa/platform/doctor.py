...
def run_doctor() -> dict[str, Any]:
    ...
    checks.append({
        ...
        "detail": "Jalankan python -m ekosistem_satwa.ml.train" if not metas else "ok",
    })
    ...
    # --- Excel exports ---
    xlsx = list(EXCEL_DIR.glob("EkosistemSatwa_*.xlsx")) if EXCEL_DIR.exists() else []
    checks.append({
        "id": "excel_exports",
        "ok": len(xlsx) > 0,
        "track": "export",
...
def _recommendations(checks: list[dict]) -> list[str]:
    ...
    if not by_id.get("ml_models", {}).get("ok"):
        rec.append("python -m ekosistem_satwa.platform.pipeline --step train_ml")
    ...
    if not by_id.get("platform_registry", {}).get("ok"):
        rec.append("python -m ekosistem_satwa.platform.registry --refresh")
    if by_id.get("learning_store", {}).get("gold_rows", 0) > 0:
        rec.append("python -m ekosistem_satwa.ml.retrain")
    if not rec:
