"""FastAPI service Ekosistem Satwa — endpoint AI untuk aplikasi dokter hewan.
...
Jalankan:
    uvicorn ekosistem_satwa.api.main:app --reload
...
class StartConsultationRequest(BaseModel):
    ...
    consultation_id: str | None = Field(
        default=None,
        description=(
            "ID sesi dari app Ekosistem Satwa utama (opsional). "
...
class ConsultationResponse(BaseModel):
    ...
    entities: dict | None = Field(
        default=None,
        description="Bundle ID entitas Ekosistem Satwa (vet, pelanggan, pet, case, ...)",
    )
...
def health_full() -> dict:
    ...
    vision = get_vision_service().capabilities()
    return {
        "status": "ok",
...
_UPLOAD_DIR = Path(os.environ.get("EKOSISTEM_SATWA_UPLOAD_DIR", "data/uploads"))
...
_EXCEL_NAME = re.compile(r"^EkosistemSatwa_\d{2}_[\w]+\.xlsx$")
...
def list_excel_exports() -> dict:
    ...
    files = []
    for path in sorted(EXCEL_DIR.glob("EkosistemSatwa_*.xlsx")):
        files.append({
            "filename": path.name,
...
def download_excel(filename: str) -> FileResponse:
    """Unduh workbook Excel (hanya EkosistemSatwa_XX_*.xlsx)."""
