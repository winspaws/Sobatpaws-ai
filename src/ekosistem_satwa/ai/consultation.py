"""Orkestrator sesi konsultasi — perekat seluruh komponen AI Ekosistem Satwa.
...
State sesi sekarang disimpan secara persistent via SessionStore (bukan in-memory saja).
Backend tersedia: jsonl (development), PostgreSQL (produksi), atau keduanya.
"""
...
def _state_to_dict(state: ConsultationState) -> dict[str, Any]:
    """Serialize ConsultationState ke dict untuk disimpan ke SessionStore."""
    return {
        "consultation_id": state.consultation_id,
        "context": state.context.model_dump(mode="json"),
        "intakes": [i.model_dump(mode="json") for i in state.intakes],
        "suggestions": [s.model_dump(mode="json") for s in state.suggestions],
        "accumulated_symptoms": dict(state.accumulated_symptoms),
    }


def _dict_to_state(data: dict[str, Any]) -> ConsultationState | None:
    """Reconstruct ConsultationState dari dict yang disimpan.

    Returns None jika data tidak lengkap atau tidak valid.
    """
    try:
        ...
        context = ConsultationContext(**ctx_data)

        intakes: list[IntakeResult] = []
        for i_data in data.get("intakes") or []:
            intakes.append(IntakeResult(**i_data))

        suggestions: list[AISuggestion] = []
        for s_data in data.get("suggestions") or []:
            suggestions.append(AISuggestion(**s_data))

        accumulated = data.get("accumulated_symptoms") or {}
        ...
        return ConsultationState(
            ...
            accumulated_symptoms=dict(accumulated),
        )
...
class ConsultationService:
    ...
    State sesi disimpan secara persistent ke SessionStore (bukan in-memory saja).
    Backend: jsonl (default) atau PostgreSQL via SESSION_STORE_BACKEND env var.
    """
    ...
    def _save_state(self, state: ConsultationState) -> None:
        """Simpan state ke persistent store."""
        data = _state_to_dict(state)
        self._session_store.save(state.consultation_id, data)

    def _load_state(self, consultation_id: str) -> ConsultationState | None:
        """Load state dari persistent store.

        Urutan: in-memory cache -> PostgreSQL -> JSON file.
        """
        data = self._session_store.load(consultation_id)
        if data is None:
            return None
    ...
    def start(
        ...
        # Simpan state awal ke persistent store
    ...
    def _run_turn(
        ...
        # Simpan state yang diperbarui ke persistent store
        self._save_state(state)
    ...
    def get_state(self, consultation_id: str) -> ConsultationState | None:
        # Coba dari SessionStore terlebih dahulu (cepat karena ada cache)
        state = self._load_state(consultation_id)
        if state:
            return state
        # Fallback: hydrate dari legacy sources (registry + learning store)
        return self._hydrate_session(consultation_id)

    def _hydrate_session(self, consultation_id: str) -> ConsultationState | None:
        """Pulihkan sesi dari persistent store atau fallback ke legacy sources.

        Urutan prioritas:
        1. SessionStore (jsonl / postgres) - state lengkap
        2. IdentityRegistry + LearningStore - rekonstruksi parsial (fallback)
        """
        # Coba SessionStore dulu (sudah dicoba di get_state, tapi cek lagi
        # untuk skenario di mana _hydrate_session dipanggil langsung)
        state = self._load_state(consultation_id)
        if state:
            return state

        # Fallback: rekonstruksi dari identity registry + learning store
        # (kompatibilitas dengan data sebelum SessionStore diimplementasikan)
        ...
        # Simpan state yang direkonstruksi ke SessionStore untuk akses cepat berikutnya
        self._save_state(state)
        return state
